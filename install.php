    ...
    <div class="row">
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            ...
            <?php elseif (isset($_GET['start'])): ?>
                <?php if (!@is_file(__TYPECHO_ROOT_DIR__ . '/config.inc.php')) : ?>
                <h1 class="typecho-install-title"><?php _e('安装失败!'); ?></h1>
                <div class="typecho-install-body">
                    <form method="post" action="?config" name="config">
                        <p class="message error"><?php _e('您没有上传 config.inc.php 文件，请您重新安装！'); ?><button class="btn primary" type="submit"><?php _e('重新安装 &raquo;'); ?></button></p>
                ...
                <h1 class="typecho-install-title"><?php _e('安装失败!'); ?></h1>
                <div class="typecho-install-body">
                    <form method="post" action="?start" name="check"><?php
                        if (($type == 'Mysql' && ($code == 1050 || $code == '42S01')) ||
                        ($type == 'SQLite' && ($code == 'HY000' || $code == 1)) ||
                        ($type == 'Pgsql' && $code == '42P07')) {
                            if (_r('delete')) {
                                //删除原有数据
                ...
                ?>
                <?php endif; ?>
            <?php elseif (isset($_GET['config'])): ?><?php
            ...
            ?>
                <form method="post" action="?config" name="config">
                    ...
                    <div class="typecho-install-body">
                        <h2><?php _e('数据库配置'); ?></h2><?php
                            if (_r('action') == 'config') {
                                $success = true;

                                if (_r('created') && !is_file('./config.inc.php')) {
...
";
                        ...
                        <div class="message notice">
                            <p><?php _e('安装程序无法自动创建 <strong>config.inc.php</strong> 文件'); ?><br /><?php _e('您可以在网站根目录下手动创建 <strong>config.inc.php</strong> 文件, 并复制如下代码至其中'); ?></p>
                            <p><textarea rows="5" onmouseover="this.select();" class="w-100 mono" readonly><?php echo htmlspecialchars($contents); ?></textarea></p>
                            <p><button name="created" value="1" type="submit" class="btn primary">创建完毕, 继续安装 &raquo;</button></p>
                        </div><?php
                                    } else {
                                        header('Location: ./install.php?start');
                                        exit;
                                ...
                                // 安装不成功删除配置文件
                                if($success != true && is_file(__TYPECHO_ROOT_DIR__ . '/config.inc.php')) {
