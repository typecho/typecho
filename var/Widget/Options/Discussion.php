...
class Discussion extends Options implements ActionInterface
{
    ...
    public function form(): Form
    {
        ...
        $commentsListSize->input->setAttribute('min', '0');
        ...
        $commentsShowOptions = [
            ...
            'commentsPageBreak'       => _t('启用分页, 并且每页显示 %s 篇评论, 在列出时将 %s 作为默认显示',
                '</label><input type="number" min="0" value="' . $this->options->commentsPageSize
            ...
            'commentsThreaded'        => _t('启用评论回复, 以 %s 层作为每个评论最多的回复层数',
                    '</label><input name="commentsMaxNestingLevels" type="number" min="0" class="text num form-sm" value="' . $this->options->commentsMaxNestingLevels . '" id="commentsShow-commentsMaxNestingLevels" />
        ...
        $commentsPostOptions = [
            ...
            'commentsAutoClose'          => _t('在文章发布 %s 天以后自动关闭评论',
                '</label><input name="commentsPostTimeout" type="number" min="0" class="text num form-sm" value="' . intval($this->options->commentsPostTimeout / (24 * 3600)) . '" id="commentsPost-commentsPostTimeout" />
            <label for="commentsPost-commentsPostTimeout">'),
            'commentsPostIntervalEnable' => _t('同一 IP 发布评论的时间间隔限制为 %s 分钟',
                '</label><input name="commentsPostInterval" type="number" min="0" class="text num form-sm" value="' . round($this->options->commentsPostInterval / (60), 1) . '" id="commentsPost-commentsPostInterval" />
