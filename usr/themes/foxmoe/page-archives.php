...
foreach ($postsData as $p) {
  ...
  if ($selectedYear) {
    if ($y === $selectedYear) $eligibleCids[] = $p['cid'];
  } else {
    $eligibleCids[] = $p['cid'];
  }
}
...
if (($categorySlug || $tagSlug) && !empty($cidAfterYear)) {
  ...
  foreach ($rows as $r) {
    $cid = (int)$r['cid'];
    if ($r['type'] === 'category') {
      if (!isset($postCats[$cid])) $postCats[$cid] = [];
      $postCats[$cid][] = $r['slug'];
    } elseif ($r['type'] === 'tag') {
      if (!isset($postTags[$cid])) $postTags[$cid] = [];
      $postTags[$cid][] = $r['slug'];
    }
  }
...
$allowedCidSet = [];
foreach ($cidAfterCatTag as $cid) { $allowedCidSet[$cid] = true; }
...
$totalCount = 0;
foreach ($postsData as $p) {
  if (!isset($allowedCidSet[$p['cid']])) continue;
  $y = (int)date('Y', $p['created']);
  $m = (int)date('n', $p['created']);
  $yearCounts[$y] = isset($yearCounts[$y]) ? $yearCounts[$y] + 1 : 1;
  if (!isset($monthCounts[$y])) $monthCounts[$y] = [];
...
if (!empty($cidAfterYear)) {
  ...
  $baseSet = [];
  foreach ($base as $cid) $baseSet[$cid] = true;
  foreach ($postsData as $p) {
    if (!isset($baseSet[$p['cid']])) continue;
...
function build_query_keep($overrides = [], $removes = []) {
  ...
  foreach ($removes as $k) { unset($params[$k]); }
          ...
          <div class="filter-bar">
            <div class="filter-row">
              ...
              <div class="filter-options">
                ...
                <a class="filter-chip <?php if(!$selectedYear) echo 'active'; ?>" href="<?php $this->permalink(); ?><?php echo $qs ? ('?'.$qs) : ''; ?>">全部</a>
                <?php foreach ($yearsForBar as $yy): ?>
                  <?php $qs = build_query_keep(['year' => $yy], []); ?>
                  <a class="filter-chip <?php if($selectedYear === (int)$yy) echo 'active'; ?>" href="<?php $this->permalink(); ?>?<?php echo $qs; ?>"><?php echo $yy; ?></a>
                <?php endforeach; ?>
              </div>
            </div>
            ...
            <div class="filter-row">
              ...
              <div class="filter-options">
                ...
                <a class="filter-chip <?php if(!$categorySlug) echo 'active'; ?>" href="<?php $this->permalink(); ?><?php echo $qs ? ('?'.$qs) : ''; ?>">全部</a>
                <?php \Widget\Metas\Category\Rows::alloc()->to($categories); ?>
                <?php while ($categories->next()): ?>
                  <?php $slug = $categories->slug; $name = $categories->name; ?>
                  <?php $qs = build_query_keep(['cat' => $slug], []); ?>
                  <a class="filter-chip <?php if($categorySlug === $slug) echo 'active'; ?>" href="<?php $this->permalink(); ?>?<?php echo $qs; ?>"><?php echo htmlspecialchars($name); ?></a>
                <?php endwhile; ?>
              </div>
            </div>
            ...
            <div class="filter-row">
              ...
              <div class="filter-options">
                ...
                <a class="filter-chip <?php if(!$tagSlug) echo 'active'; ?>" href="<?php $this->permalink(); ?><?php echo $qs ? ('?'.$qs) : ''; ?>">全部</a>
                <?php \Widget\Metas\Tag\Cloud::alloc()->to($tags); ?>
                <?php while ($tags->next()): ?>
                  <?php $slug = $tags->slug; $name = $tags->name; ?>
                  <?php $qs = build_query_keep(['tg' => $slug], []); ?>
                  <a class="filter-chip <?php if($tagSlug === $slug) echo 'active'; ?>" href="<?php $this->permalink(); ?>?<?php echo $qs; ?>"><?php echo htmlspecialchars($name); ?></a>
                <?php endwhile; ?>
              </div>
            </div>
          </div>
