...
function themeConfig($form)
{
    ...
    $sidebarBlock = new \Typecho\Widget\Helper\Form\Element\Checkbox(
        ...
        [
            'ShowAbout' => _t('显示 关于我'),
            'ShowCount' => _t('显示 访问人数'),
            'ShowRecommend' => _t('显示 推荐阅读'),
            'ShowTagCloud' => _t('显示 标签云'),
            'ShowCategories' => _t('显示 分类目录'),
            'ShowRecentComments' => _t('显示 最新评论'),
        ],
        ['ShowAbout', 'ShowCount', 'ShowRecommend', 'ShowTagCloud', 'ShowCategories', 'ShowRecentComments'],
        _t('侧边栏显示')
    );
    ...
    // 网站访问人数配置
    $enterCount = new \Typecho\Widget\Helper\Form\Element\Text(
        'enterCount',
        NULL,
        '1',
        _t('网站访问人数'),
    ...
    $hitokotoUA = new \Typecho\Widget\Helper\Form\Element\Text(
        'hitokotoUA',
        NULL,
        'FoxmoeTheme/1.1 (+https://foxmoe.top)',
        _t('Hitokoto 请求 UA'),
        _t('请求一言时使用的 User-Agent')
    );
    ...
    $githubImageProxy = new \Typecho\Widget\Helper\Form\Element\Text(
        ...
        'https://get.2sb.org/',
        _t('Github 图床加速源'),
        _t('例如 https://get.2sb.org/ ，末尾保留 / ，留空则不加速；会在文章缩略图/列表中作为前缀拼接字段值')
    );
    ...
    $autoThemeNightStart = new \Typecho\Widget\Helper\Form\Element\Text(
        ...
        '18:00',
        _t('自动切换主题-夜晚开始时间'),
        _t('格式 HH:MM，默认 18:00；夜晚开始到次日白天开始为深色模式')
    );
    ...
    $autoThemeDayStart = new \Typecho\Widget\Helper\Form\Element\Text(
        ...
        '06:00',
        _t('自动切换主题-白天开始时间'),
        _t('格式 HH:MM，默认 06:00；白天开始到夜晚开始为浅色模式')
    );
 ...
 * 增加网站访问人数 enterCount
 */
function increment_site_enter_count($cookieName = 'foxmoe_enter_count', $expire = 3600)
{
    if (isset($_COOKIE[$cookieName]) && $_COOKIE[$cookieName]) {
        $opts = Helper::options();
        return isset($opts->enterCount) ? intval($opts->enterCount) : 0;
    }
    ...
    $optsRuntime = Helper::options();
    $current = isset($optsRuntime->enterCount) ? intval($optsRuntime->enterCount) : 0;
    $newCount = $current + 1;
    ...
    if ($row && isset($row['value'])) {
        ...
        $value['enterCount'] = strval($newCount);

        try {
            $db->query($db->update('table.options')->rows(array('value' => serialize($value)))->where('name = ?', 'theme:' . $themeName));
        } catch (Exception $e) {
        }
    }

    setcookie($cookieName, '1', time() + $expire, '/');
