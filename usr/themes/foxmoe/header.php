...
<head>
  ...
  <!-- 预加载 -->
  <link rel="preload" href="<?php $this->options->themeUrl('fonts/MaterialIcons-Regular.woff2'); ?>" as="font"
    type="font/woff2" crossorigin>
  <link rel="preload" href="<?php $this->options->themeUrl('fonts/InterVariable.woff2'); ?>" as="font" type="font/woff2"
    crossorigin>
</head>
        ...
        <div class="nav-right">
          <ul class="nav-menu" id="mainNavMenu" aria-label="主导航">
            <li><a href="<?php $this->options->siteUrl(); ?>"
                class="nav-link<?php if ($this->is('index')): ?> active<?php endif; ?>">首页</a></li>
            <?php \Widget\Contents\Page\Rows::alloc()->to($pages); ?>
            <?php while ($pages->next()): ?>
              <li><a href="<?php $pages->permalink(); ?>"
                  class="nav-link<?php if ($this->is('page') && $this->slug == $pages->slug): ?> active<?php endif; ?>"><?php $pages->title(); ?></a>
              </li>
            <?php endwhile; ?>
            <!-- 移动端菜单内搜索区域 -->
            <li class="nav-search" id="mobileMenuSearch" hidden>
              <form method="post" action="<?php $this->options->siteUrl(); ?>" role="search" aria-label="站内搜索 (移动菜单)">
                <input type="text" name="s" class="search-input" placeholder="搜索..." value="<?php if (isset($_POST['s']))
                  echo htmlspecialchars($_POST['s']); ?>" />
                <button type="submit" class="search-submit" aria-label="开始搜索"><span
                    class="material-icons">search</span></button>
              </form>
            </li>
          </ul>
          <div class="nav-actions">
            <button type="button" class="search-btn" id="globalSearchBtn" aria-label="打开搜索" aria-haspopup="true"
              aria-expanded="false">
            ...
            <button type="button" class="sidebar-toggle-btn" id="sidebarToggleBtn" aria-label="打开侧栏"
              aria-expanded="false">
